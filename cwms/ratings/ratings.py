...
def get_current_rating(rating_id: str, office_id: str, rating_table_in_df: Optional[bool] = True):
    ...
    ratings = get_ratings(
        rating_id=rating_id, office_id=office_id, method="LAZY")

    # find the most recent effective date
    max_effective = pd.to_datetime(ratings.df['effective-date']).max()

    rating = get_ratings(rating_id=rating_id, office_id=office_id,
                         begin=max_effective, end=max_effective, method="EAGER", rating_table_in_df=rating_table_in_df)

    return rating


def get_ratings(rating_id: str, office_id: str, begin: Optional[datetime] = None,
                end: Optional[datetime] = None, timezone: Optional[str] = None, method: Optional[str] = 'EAGER',
                rating_table_in_df: Optional[bool] = False) -> Data:
    ...
    if rating_table_in_df and method == 'EAGER':
        data = Data(response, selector='simple-rating.rating-points.point')
    else:
