...
class TestSpecLevels(unittest.TestCase):
    ...
    def test_retrieve_specified_levels_json_default(self, m):
        m.get(
            f"{TestSpecLevels._MOCK_ROOT}/specified-levels?office=%2A&template-id-mask=%2A",
    ...
    def test_retrieve_specified_levels_json(self, m):
        m.get(
            f"{TestSpecLevels._MOCK_ROOT}/specified-levels?office=SWT&template-id-mask=%2A",
            json=_SPEC_LEVELS_JSON,
        )
        cwms_levels = CwmsLevel(CwmsApiSession(TestSpecLevels._MOCK_ROOT))
    ...
    def test_store_specified_level_json(self, m):
        m.post(
            f"{TestSpecLevels._MOCK_ROOT}/specified-levels?fail-if-exists=True",
            status_code=200,
    ...
    def test_store_specified_level_error_code_json(self, m):
        with pytest.raises(HTTPError):
            m.post(
                f"{TestSpecLevels._MOCK_ROOT}/specified-levels",
                status_code=500,
                json=_ERROR_CODE_500_JSON,
            )
            cwms_levels = CwmsLevel(CwmsApiSession(TestSpecLevels._MOCK_ROOT))
            cwms_levels.store_specified_level_json(_SPEC_LEVEL_JSON)
    ...
    def test_delete_specified_level_json(self, m):
        m.delete(
            f"{TestSpecLevels._MOCK_ROOT}/specified-levels/Test?office=SWT",
            status_code=200,
        )
        cwms_levels = CwmsLevel(CwmsApiSession(TestSpecLevels._MOCK_ROOT))
    ...
    def test_update_specified_level_json(self, m):
        m.patch(
            f"{TestSpecLevels._MOCK_ROOT}/specified-levels/Test?specified-level-id=TEst2&office=SWT",
            status_code=200,
        )
        cwms_levels = CwmsLevel(CwmsApiSession(TestSpecLevels._MOCK_ROOT))
...
class TestLocLevels(unittest.TestCase):
    ...
    def test_retrieve_loc_levels_json_default(self, m):
        m.get(
            f"{TestLocLevels._MOCK_ROOT}/levels?level-id-mask=%2A",
    ...
    def test_retrieve_loc_levels_json(self, m):
        m.get(
            f"{TestLocLevels._MOCK_ROOT}/levels?office=SWT&level-id-mask=AARK.Elev.Inst.0.Bottom+of+Inlet&"
    ...
    def test_retrieve_loc_level_json(self, m):
        m.get(
            f"{TestLocLevels._MOCK_ROOT}/levels/AARK.Elev.Inst.0.Bottom%20of%20Inlet?office=SWT&"
    ...
    def test_delete_loc_level(self, m):
        ...
        timezone = pytz.timezone("US/Pacific")
        effective_date = timezone.localize(datetime(2020, 2, 14, 10, 30, 0))
        cwms_levels.delete_location_level(level_id, office_id, effective_date, True)
        assert m.called
        assert m.call_count == 1
    ...
    def test_retrieve_loc_level_ts_json(self, m):
        m.get(
            f"{TestLocLevels._MOCK_ROOT}/levels/AARK.Elev.Inst.0.Bottom%20of%20Inlet/timeseries?office=SWT&unit=m&"
